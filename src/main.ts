...
import { AiAssisterPanel, makePanelResizable } from '@/ui/ai-assister-panel';
import { AiInlineToolbar } from '@/ui/ai-inline-toolbar';
import { HwpCtrl } from '@/hwpctl';
import { insertFormattedTextToHwp } from '@/core/ai-formatter';
...
// PWA 서비스 워커 자동 등록 및 업데이트 감지
registerSW({
  immediate: true,
  onNeedRefresh() {
    console.log('[PWA] 새로운 버전을 사용할 수 있습니다.');
  },
  onOfflineReady() {
    console.log('[PWA] 오프라인 상태에서 사용할 준비가 되었습니다.');
  },
});

// 브라우저 PWA 프롬프트(주소 표시줄 설치 및 인앱 설치) 이벤트 캐칭
let deferredInstallPrompt: any = null;
window.addEventListener('beforeinstallprompt', (e: Event) => {
  e.preventDefault();
  deferredInstallPrompt = e;
  (window as any).__deferredInstallPrompt = deferredInstallPrompt;
  console.log('[PWA] 주소 표시줄 및 브라우저 PWA 앱 설치 이벤트 활성화');
});
...
async function initializeDocument(
  ...
  try {
    ...
    // 새 문서 열림/창 갱신 시 AI Assister 대화 기록 및 컨텍스트 완전 초기화
    aiPanelInstance?.resetState(wasm.fileName || '새 문서');
  } catch (error) {
...
let hwpCtrlInstance: HwpCtrl | null = null;
let aiPanelInstance: AiAssisterPanel | null = null;

function getHwpCtrl(): HwpCtrl | null {
  if (!wasm) return null;
  if (!hwpCtrlInstance) {
    hwpCtrlInstance = new HwpCtrl(wasm);
  }
  // 문서가 비어있으면 자동으로 빈 문서 생성하여 즉시 편집 가능하도록 보장
  if (wasm.pageCount === 0) {
    try {
      wasm.createNewDocument();
    } catch (e) {
      console.warn('[getHwpCtrl] 빈 문서 자동 생성 예외 무시:', e);
    }
  }
  // inputHandler의 현재 커서 위치를 HwpCtrl에 반영
  if (inputHandler) {
    const pos = inputHandler.getCursorPosition();
    if (pos) {
      hwpCtrlInstance.SetPos(pos.sectionIndex ?? 0, pos.paragraphIndex ?? 0, pos.charOffset ?? 0);
    }
  }
  return hwpCtrlInstance;
}
...
function initAiAssister() {
  ...
  if (mainWrap) {
    ...
    resizer.title = '드래그하여 AI Assister 패널 폭 조절';
    mainWrap.appendChild(resizer);
    mainWrap.appendChild(aiPanelInstance.getElement());

    makePanelResizable(resizer, aiPanelInstance.getElement(), () => {
      window.dispatchEvent(new Event('resize'));
    });
  }

  const inlineToolbar = new AiInlineToolbar(ollamaClient, (replacementText, actionType) => {
    const ctrl = getHwpCtrl();
    if (!ctrl) {
      showToast('에디터가 준비되지 않았습니다.');
      return;
    }

    if (actionType === 'replace') {
      insertFormattedTextToHwp(ctrl, replacementText);
      showToast('AI 추천 문장으로 적용되었습니다.');
    } else if (actionType === 'append') {
      insertFormattedTextToHwp(ctrl, ` ${replacementText}`);
      showToast('AI 추천 문장이 추가되었습니다.');
    }
  ...
  document.addEventListener('mousedown', (e) => {
    const target = e.target as HTMLElement;
    if (!target.closest('.ai-inline-toolbar') && !target.closest('.ai-diff-modal')) {
      inlineToolbar.hide();
    }
  });
}
