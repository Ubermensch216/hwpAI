...
export function showToast(options: ToastOptions | string): void {
  const opt: ToastOptions = typeof options === 'string' ? { message: options } : options;
  const container = ensureContainer();
  const duration = opt.durationMs ?? DEFAULT_DURATION_MS;
  ...
  if (opt.action) {
    ...
    actionBtn.textContent = opt.action.label;
    ...
    actionBtn.addEventListener('click', () => {
      ...
      if (!opt.confirmLabel) removeToast();
    });
  ...
  if (opt.confirmLabel) {
    ...
    confirmBtn.textContent = opt.confirmLabel;
